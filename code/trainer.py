...
class Trainer():
    ...
    def test(self):
        ...
        for idx_scale, scale in enumerate(self.scale):
            ...
            for idx_img, (input, target, _) in enumerate(self.loader_test):
                ...
                try:
                    ...
                    validation_loss += self._calc_loss_for_validation(output, target)
                except:

                    sys.stdout = TooLargeImageslog_file
                    print("image: ", idx_img + 1 + self.args.n_train)     # note: idx_img + 1 + self.args.n_train (used to be just idx_img)
                    sys.stdout = old_stdout

            self.ckp.log_test_loss[-1, idx_scale] = validation_loss / len(self.loader_test)
        ...
        self.ckp.save(self, epoch, is_best=is_best) #this is where we save (in utils) , in save function we also plot.
    ...
    def _calc_loss_for_validation(self, output, target):
        loss_list = []
        ...
        for i, l in enumerate(self.loss):
            ...
            loss_list.append(l['weight'] * loss)
            #self.ckp.log_test_loss[-1, i] += loss.data[0]

        loss_total = reduce((lambda x, y: x + y), loss_list)
        #if len(self.loss) > 1:
            #self.ckp.log_test_loss[-1, -1] += loss_total.data[0]

        return loss_total.data[0] #if fails here - return loss_total.data instead
