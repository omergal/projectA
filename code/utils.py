...
class checkpoint():
    def __init__(self, args):
        self.args = args
        self.ok = True
        self.log_test_loss = torch.Tensor()
    ...
    def load(self):
        ...
        if self.args.load == '.':
            ...
            if not self.args.test_only:
                ...
                self.log_test_loss = torch.load(self.dir + '/log_test_loss.pt')
    ...
    def add_log(self, log, train=True):
        if train:
            ...
            self.log_test_loss = torch.cat([self.log_test_loss, log])
    ...
    def save(self, trainer, epoch, is_best=False):
        ...
        if not self.args.test_only:
            ...
            torch.save(
                self.log_test_loss,
                self.dir + '/log_test_loss.pt')
            self.plot(trainer, epoch, self.log_training, self.log_test, self.log_test_loss)
    ...
    def plot(self, trainer, epoch, training, test, test_loss):
        ...
        for i, loss in enumerate(trainer.loss):
            ...
            plt.plot(axis, training[:, i].numpy(), label="training loss")
            plt.plot(axis, test_loss[:, i].numpy(), label="validation loss")
            plt.legend()
        ...
        fig = _init_figure('PSNR of DeJpg2000 on {}'.format(set_name))
        for idx_scale, scale in enumerate(self.args.scale):
            #legend = 'Scale {}'.format(scale)
            plt.plot(axis, test[:, idx_scale].numpy(), label="validation")
            #plt.plot(axis, training[:, idx_scale].numpy(), label="training")
            plt.legend()
